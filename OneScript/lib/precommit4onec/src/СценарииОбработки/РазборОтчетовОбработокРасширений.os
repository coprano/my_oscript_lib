...
Функция ОбработатьФайл(АнализируемыйФайл, КаталогИсходныхФайлов, ДополнительныеПараметры) Экспорт
	...
	Если ТипФайлаПоддерживается(АнализируемыйФайл) Тогда
		...
		Если ФайлыВКаталогеТВФ.Количество() = 0 Тогда
			// Возникла ошибка при распаковке файла, сценарий не выполнен
			Возврат Ложь;
		КонецЕсли;

		Для Каждого ФайлВКаталогеТВФ Из ФайлыВКаталогеТВФ Цикл
			ДополнительныеПараметры.ФайлыДляПостОбработки.Добавить(ФайлВКаталогеТВФ.ПолноеИмя);
		КонецЦикла;
...
Процедура РаспаковатьОтчетОбработку(Знач Файл, Знач КаталогВыгрузки, ИспользуемаяВерсияПлатформы)
	...
	Исключение
		Лог.Ошибка("Возникла ошибка при выполнении команды конфигуратора: `%1`", ОписаниеОшибки());
	КонецПопытки;
