...
def force_recalculation(workbook_xml_data, debug=False, full_calc_on_load=False):
    ...
    try:
        ...
        # Check if calcPr element exists
        calcpr_pattern = r'<calcPr[^>]*(?:/>|>.*?</calcPr>)'
        calcpr_match = re.search(calcpr_pattern, xml_str, re.DOTALL)
        ...
        if debug:
            print(f"  Warning: Could not modify calcPr: {e}")
            ...
            with zipfile.ZipFile(input_path, 'r') as src_zip:
                with zipfile.ZipFile(temp_output, 'w', zipfile.ZIP_DEFLATED) as dst_zip:
                    ...
                    for item in src_zip.namelist():
                        # Skip calcChain.xml - Excel will regenerate it on first open
                        # This triggers one-time recalculation when --recalculate flag is used
                        ...
                        # Modify workbook.xml to force recalculation
...
Examples:
  ...
  # Skip formula recalculation
  %(prog)s --excel-file input.xlsx --json-file commands.json -o output.xlsx --no-recalculate
