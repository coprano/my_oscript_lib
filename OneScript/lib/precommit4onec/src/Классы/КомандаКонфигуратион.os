...
Функция ИнтерактивнаяНастройка(КаталогРепозитория)
	...
	Если ИнтерактивныйРедактор.ПолучитьНастройкуБулево("Проверять сообщение коммита на наличие номера задачи?", Ложь) Тогда
		ШаблонВыражения = ИнтерактивныйРедактор.ПолучитьНастройкуСтрока("Укажите регулярное выражение для проверки строки сообщения: ",
			МенеджерНастроек.ЗначениеНастройки("ПроверкаСообщенияКоммита.Выражение"));
		СообщениеОбОшибке = ИнтерактивныйРедактор.ПолучитьНастройкуСтрока("Введите сообщение об ошибке, которое будет отображаться пользователю: ",
			МенеджерНастроек.ЗначениеНастройки("ПроверкаСообщенияКоммита.СообщениеОбОшибке"));
	Иначе
		ШаблонВыражения = "";
		СообщениеОбОшибке = "";
	КонецЕсли;
	...
	НастройкиПриложения.Вставить("ПроверкаСообщенияКоммита", НастройкиПроверкиСообщенияКоммита(ШаблонВыражения, СообщениеОбОшибке));
...
Функция НастройкиПроверкиСообщенияКоммита(ШаблонВыражения, СообщениеОбОшибке)
	Возврат Новый Структура("Использовать, Выражение, СообщениеОбОшибке",
		НЕ ПустаяСтрока(ШаблонВыражения), ШаблонВыражения, СообщениеОбОшибке);
КонецФункции
